...
//
    ...
    class func isGameEarly(p:Packet, timeSpan:Double, maxPacket:Int, packetNumber:Int) -> Bool{
        ...
        while(!dstGameTimer.isEmpty && p.captureTime - dstGameTimer.last!.time > timeSpan){
        ...
        for key in dstPacketCounter.keys{
            if(p.packetLength <= key + 5 && p.packetLength >= key && p.dstPort == queuePort){
